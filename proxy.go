...
// ProxyDialer is a dialer that connects to the address addr on the network net via the SOCKS5 proxy.
...
// GetProxyDialer returns a proxy dialer
func GetProxyDialer(c *ShadowConfig) (*ProxyDialer, error) {
	ciph, err := core.PickCipher(c.Cipher, nil, c.Password)
	if err != nil {
		return nil, err
	}

	if c.Plugin != "" {
		c.Addr, err = startPlugin(c.Plugin, c.PluginParam, c.Addr, false)
	}
